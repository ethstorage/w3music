...
contract W3MusicNft is ERC721('W3MUSIC', 'MUSIC'), FlatDirectory(0) {
    struct MusicInfo {
        uint256 createTime;
        address author;
        bytes fileName;
    ...
    struct TokenInfo {
        uint256[] tokenIds;
        mapping(uint256 => uint256) tokenIndexes; // tokenid => inddex
    }

    mapping(uint256 => MusicInfo) musicInfos;
    mapping(address => TokenInfo) userTokens;
    ...
    function mint(
        ...
        TokenInfo storage tokenInfo = userTokens[author_];
        tokenInfo.tokenIds.push(tokenId);
        tokenInfo.tokenIndexes[tokenId] = tokenInfo.tokenIds.length;
        totalSupply++;
    }
    ...
    function _transfer(
        ...
        TokenInfo storage fromInfo = userTokens[from];
        uint256 lastIndex = fromInfo.tokenIds.length - 1;
        uint256 removeIndex = fromInfo.tokenIndexes[tokenId] - 1;
        if (lastIndex != removeIndex) {
            uint256 lastTokenId = fromInfo.tokenIds[lastIndex];
            fromInfo.tokenIds[removeIndex] = lastTokenId;
            fromInfo.tokenIndexes[lastTokenId] = removeIndex + 1;
        }
        fromInfo.tokenIds.pop();
        delete fromInfo.tokenIndexes[tokenId];

        TokenInfo storage toInfo = userTokens[to];
        toInfo.tokenIds.push(tokenId);
        toInfo.tokenIndexes[tokenId] = toInfo.tokenIds.length - 1;
    }
    ...
    {
        uint256 length = userTokens[author].tokenIds.length;
        ...
        for (uint256 i; i < length; i++) {
            uint256 tokenId = userTokens[author].tokenIds[i];
            ids[i] = tokenId;
